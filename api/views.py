...
from .serializers import StudentSerializers, StudentNotesSerializers, UpdateStudentNotesSerializers
...
class UpdateStudentNotesAPiView(APIView):
    def post(self, request):
        serializer = UpdateStudentNotesSerializers(data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response(serializer.data, status=status.HTTP_201_CREATED)